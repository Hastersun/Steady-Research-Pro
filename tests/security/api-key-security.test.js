...
describe('API密钥安全存储测试', () => {
  ...
  test('API密钥不应明文存储', () => {
    const sensitiveData = {
      bing: 'real-api-key-123',
      google: 'AIzaSyDGvf8N9X7mPqK2L1A5b6C7d8E9f0G1h2I3',
    };
  ...
  test('API密钥格式验证', () => {
    ...
    // 测试有效的Google密钥格式
    expect(() =>
      validateApiKey('AIzaSyDGvf8N9X7mPqK2L1A5b6C7d8E9f0G1h2I3', 'google')
    ).not.toThrow();
  ...
  test('恶意脚本无法读取真实密钥', () => {
    ...
    const hasPlaintextKey = allStorageValues.some(
      value => typeof value === 'string' && value.includes('secret-api-key-789')
    );
